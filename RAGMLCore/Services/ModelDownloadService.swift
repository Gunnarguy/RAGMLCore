...
extension ModelDownloadService {
    ...
    static func huggingFaceToken() -> String? {
        if let raw = UserDefaults.standard.string(forKey: "hfToken")?.trimmingCharacters(in: .whitespacesAndNewlines),
           !raw.isEmpty {
            return raw
        }
        return nil
...
extension ModelDownloadService {
    ...
    func listHuggingFaceGGUFFiles(owner: String, repo: String, token: String?, includeCoreML: Bool = false) async throws -> [HFSibling] {
        var comps = URLComponents()
        comps.scheme = "https"
        comps.host = "huggingface.co"
        ...
        if let http = resp as? HTTPURLResponse, !(200...299).contains(http.statusCode) {
            if http.statusCode == 403 {
                throw NSError(domain: "HF", code: 403, userInfo: [NSLocalizedDescriptionKey: "Access denied. If the model is gated, accept the license on huggingface.co and set a token in Settings."])
            }
            if http.statusCode == 404 {
                throw NSError(domain: "HF", code: 404, userInfo: [NSLocalizedDescriptionKey: "Repo not found: \(owner)/\(repo)"])
            }
            if http.statusCode == 429 {
                throw NSError(domain: "HF", code: 429, userInfo: [NSLocalizedDescriptionKey: "Rate limited by Hugging Face. Try again later."])
            }
            throw NSError(domain: "HF", code: http.statusCode, userInfo: [NSLocalizedDescriptionKey: "HTTP \(http.statusCode)"])
        }
        ...
        let filtered = files.filter { sib in
            ...
            if includeCoreML && (name.hasSuffix(".mlpackage") || name.hasSuffix(".mlpackage.zip") || name.hasSuffix(".zip")) { return true }
            return false
        }
    ...
    func startHuggingFaceDownload(owner: String, repo: String, revision: String = "main", path: String, sizeBytes: Int64?, sha256: String?) {
        let lower = path.lowercased()
        let backend: ModelBackend = {
            if lower.hasSuffix(".gguf") { return .gguf }
            if lower.hasSuffix(".mlpackage") || lower.hasSuffix(".mlpackage.zip") || lower.hasSuffix(".zip") { return .coreML }
            return .gguf
        }()
...
final class ModelDownloadService: NSObject, ObservableObject {
    ...
    private func buildSession() -> URLSession {
        ...
        let wifiOnly = UserDefaults.standard.bool(forKey: "modelsWiFiOnly")
    ...
    /// Rebuild the URLSession to apply updated Settings (e.g., Wi‑Fi only)
    func rebuildSession() {
    ...
    private static func fallbackCatalog() -> [ModelCatalogEntry] {
        ...
        return [
            ...
            ModelCatalogEntry(
                ...
                backend: .gguf,
                url: "hf://lmstudio-community/Llama-3.2-1B-Instruct-GGUF:main/Llama-3.2-1B-Instruct-Q4_K_M.gguf",
            ...
            ModelCatalogEntry(
                ...
                backend: .gguf,
                url: "hf://lmstudio-community/Llama-3.2-3B-Instruct-GGUF:main/Llama-3.2-3B-Instruct-Q4_K_M.gguf",
            ...
            ModelCatalogEntry(
                ...
                backend: .gguf,
                url: "hf://bartowski/Phi-3.5-mini-instruct-GGUF:main/Phi-3.5-mini-instruct-Q4_K_M.gguf",
    ...
    func download(entry: ModelCatalogEntry) {
        ...
        if entry.url.lowercased().hasPrefix("hf://"),
           ...
           let hfReq = Self.huggingFaceResolveRequest(for: ref, token: Self.huggingFaceToken()) {
            ...
            TelemetryCenter.emit(.error, severity: .error, title: "Model download failed", metadata: ["reason": "Invalid URL", "name": entry.name])
            return
        }
        ...
        if !ok, let need = entry.sizeBytes {
            ...
            updateDownload(entry.id, status: .failed(error: "Not enough free space (need \(needMB), have \(haveMB))"))
            TelemetryCenter.emit(
                .storage,
                severity: .error,
    ...
    private func finalizeDownload(for entryID: UUID, tempURL: URL, suggestedFilename: String?) {
        ...
        Task { @MainActor in
            ...
            do {
                ...
                let fileName = entry.filename ?? suggestedFilename ?? URL(string: entry.url)?.lastPathComponent ?? UUID().uuidString
                ...
                guard fm.fileExists(atPath: tempURL.path) else {
                    updateDownload(entryID, status: .failed(error: "Download incomplete - temp file missing"))
                    return
                }
                ...
                if let attrs = try? fm.attributesOfItem(atPath: tempURL.path),
                   let fileSize = attrs[.size] as? Int64 {
                    if fileSize < 1024 {
                        updateDownload(entryID, status: .failed(error: "Download incomplete - file too small (\(fileSize) bytes)"))
                ...
                if let expected = entry.checksumSHA256, !expected.isEmpty {
                    ...
                    if !ok {
                        try? fm.removeItem(at: destURL)
                        updateDownload(entryID, status: .failed(error: "Checksum verification failed"))
                        return
                    }
                    #endif
                }
                ...
                case .mlxServer:
                    // Not a file payload; this path should not be used via downloader
                    break
                }
    ...
    nonisolated func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask,
        ...
        DispatchQueue.main.async {
            guard let entryID = self.taskToEntry[taskID],
                  var st = self.downloads[entryID] else { return }
    ...
    nonisolated func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask,
        ...
        do {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self.finalizeDownload(for: entryID, tempURL: safeTempURL, suggestedFilename: filename)
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                if let entryID = self.taskToEntry[taskID] {
                    self.updateDownload(entryID, status: .failed(error: "Failed to preserve download: \(error.localizedDescription)"))
                    self.taskToEntry.removeValue(forKey: taskID)
                }
            }
        }
