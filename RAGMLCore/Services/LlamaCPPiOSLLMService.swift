...
final class LlamaCPPiOSLLMService: LLMService {
    ...
    private func resolveModelURL() throws -> URL {
        guard let url = installedModel.localURL, FileManager.default.fileExists(atPath: url.path) else {
            TelemetryCenter.emit(
                .error,
                severity: .error,
    ...
    private func makeMessages(prompt: String, context: String?) -> [LLMInput.Message] {
        var messages: [LLMInput.Message] = [
            .system("You are a helpful assistant. If context is provided, ground your answer in it and cite sources when available.")
        ]
...
private actor GGUFClientRuntime {
    ...
    func generate(
        ...
        for try await chunk in stream {
            ...
            aggregated += chunk
            LLMStreamingContext.emit(text: aggregated, isFinal: false)
        }

        let total = Date().timeIntervalSince(start)
        LLMStreamingContext.emit(text: aggregated, isFinal: true)
    ...
    private func client(for url: URL, parameter: LlamaClient.Parameter) async throws -> LlamaClient {
